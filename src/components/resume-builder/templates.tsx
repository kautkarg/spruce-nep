...
export const ModernMinimalistTemplate: React.FC<TemplateProps> = ({ resumeData }) => (
    <div className="font-sans text-xs">
        ...
        {hasContent(resumeData.experience, 'title') && <section className="mb-4"><h2 className="text-sm font-semibold tracking-wider uppercase mb-2">Experience</h2>{resumeData.experience?.map((exp, index) => exp.title && (
            <div key={index} className="mb-3 grid grid-cols-4 gap-2"><div className="col-span-1 text-gray-500"><p className="text-xs">{exp.dates}</p><p className='font-semibold text-xs'>{exp.organization}</p></div><div className="col-span-3"><h3 className="text-sm font-bold">{exp.title}</h3>{hasContent(getAchievements(exp.achievements)) && <ul className="list-disc list-outside pl-4 space-y-1">{getAchievements(exp.achievements).map((ach, i) => ach && <li key={i}>{ach}</li>)}</ul>}</div></div>
        ))}</section>}
        ...
        {hasContent(resumeData.education, 'school') && <section className="mb-4"><h2 className="text-sm font-semibold tracking-wider uppercase mb-2">Education</h2>{resumeData.education?.map((edu, index) => edu.school && (
            <div key={index} className="mb-2 grid grid-cols-4 gap-2"><div className="col-span-1 text-gray-500"><p>{edu.date}</p></div><div className="col-span-3"><h3 className="text-sm font-bold">{edu.degree}</h3><p>{edu.school}</p>{edu.scoreValue && edu.scoreType && <p className="text-xs">{edu.scoreType}: {edu.scoreValue}{edu.scoreType === 'Percentage' ? '%' : ''}</p>}</div></div>
        ))}</section>}
